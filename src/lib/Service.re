...
module Board = {
  ...
  let get_all_by_user = (request, user, ~user_id) => {
    if (!Sihl.User.is_admin(user) && !Sihl.User.is_owner(user, user_id)) {
      Sihl.Core.Err.raise_no_permissions("Not allowed");
    };
  ...
  let create = (request, user, ~title) => {
    let board = Model.Board.make(~title, ~owner=Sihl.User.id(user));
    let* _ =
...
module Issue = {
  ...
  let get_all_by_board = (request, user, ~board_id) => {
    ...
    if (!Sihl.User.is_admin(user) && !Sihl.User.is_owner(user, board.owner)) {
      Sihl.Core.Err.raise_no_permissions("Not allowed");
    };
